 ...
 */
export const DIFF_DELETE: DiffOP = -1;
export const DIFF_INSERT: DiffOP = 1;
export const DIFF_EQUAL: DiffOP = 0;
...
export class DiffMatchPatch {
	...
	static Diff_Timeout = 1.0;

	// Cost of an empty edit operation in terms of edit characters.
	static Diff_EditCost = 4;

	// At what point is no match declared (0.0 = perfection, 1.0 = very loose).
	static Match_Threshold = 0.5;
	 ...
	 */
	static Patch_DeleteThreshold = 0.5;

	// Chunk size for context length.
	static Patch_Margin = 4;

	// The number of bits in an int.
	static Match_MaxBits = 32;
	 ...
	 * by.  Used internally for recursive calls.  Users should set DiffTimeout instead.
	 */
	diffMain(text1: string, text2: string, opt_checklines?: boolean, opt_deadline?: number): Diff[] {
		// Set a deadline by which time the diff must be complete.
		if (opt_deadline === undefined) {
			if (DiffMatchPatch.Diff_Timeout <= 0) {
				opt_deadline = Number.MAX_VALUE;
			} else {
				opt_deadline = (new Date).getTime() + DiffMatchPatch.Diff_Timeout * 1000;
			}
		}
	 ...
	 */
	diffCompute(text1: string, text2: string, checklines: boolean, deadline: number): Diff[] {
		let diffs: Diff[];
		...
		if (hm) {
			...
			const diffs_a = this.diffMain(text1_a, text2_a, checklines, deadline);
			const diffs_b = this.diffMain(text1_b, text2_b, checklines, deadline);
			// Merge the results.
	 ...
	 * @returns Array of diff tuples.
	 */
	diffLineMode(text1: string, text2: string, deadline: number) {
		...
		const diffs: Diff[] = this.diffMain(text1, text2, false, deadline);
			...
			switch (diffs[pointer][0]) {
				...
				case DIFF_EQUAL:
					...
					if (count_delete >= 1 && count_insert >= 1) {
						...
						const subDiff = this.diffMain(text_delete, text_insert, false, deadline);
	 ...
	 * @returns
	 */
	diffBisect(text1: string, text2: string, deadline: number): Diff[] {
	 ...
	 * @param deadline
	 */
	diffBisectSplit(text1: string, text2: string, x: number, y: number, deadline: number) {
	 ...
	 */
	diffCharsToLines(diffs: Diff[], lineArray: string[]) {
	 ...
	 */
	diffCommonPrefix(text1: string, text2: string) {
	 ...
	 */
	diffCommonSuffix(text1: string, text2: string) {
		// Quick check for common null cases.
		if (!text1 || !text2 ||
	 ...
	 */
	diffCommonOverlap(text1: string, text2: string) {
		// Cache the text lengths to prevent multiple calls.
		const text1_length = text1.length;
		const text2_length = text2.length;
	 ...
	 * @param text1
	 * @param text2
	 * @returns Five element Array, containing the prefix of text1, the suffix of text1,
	 * the prefix of text2, the suffix of text2 and the common middle.  Or null if there was no match.
	 */
	diffHalfMatch(text1: string, text2: string): [string, string, string, string, string] | null {
		if (DiffMatchPatch.Diff_Timeout <= 0) {
		 ...
		 * of shorttext and the common middle.  Or null if there was no match.
		 */
		function diffHalfMatchI(longtext: string, shorttext: string, i: number): [string, string, string, string, string] | null {
			...
			while ((j = shorttext.indexOf(seed, j + 1)) !== -1) {
				const prefixLength = dmp.diffCommonPrefix(longtext.substring(i),
					shorttext.substring(j));
				const suffixLength = dmp.diffCommonSuffix(longtext.substring(0, i),
	...
	diffCleanupSemantic(diffs: Diff[]) {
		...
		while (pointer < diffs.length) {
			if (diffs[pointer - 1][0] === DIFF_DELETE &&
				...
				const overlap_length1 = this.diffCommonOverlap(deletion, insertion);
				const overlap_length2 = this.diffCommonOverlap(insertion, deletion);
	 ...
	 * @param diffs
	 */
	diffCleanupSemanticLossless(diffs: Diff[]) {
		/**
	 ...
	 * @param diffs
	 */
	diffCleanupEfficiency(diffs: Diff[]) {
		let changes = false;
		const equalities = [];  // Stack of indices where equalities are found.
		let equalitiesLength = 0;  // Keeping our own length var is faster in JS.
		/** @type {?string} */
		...
		while (pointer < diffs.length) {
			if (diffs[pointer][0] === DIFF_EQUAL) {  // Equality found.
				if (diffs[pointer][1].length < DiffMatchPatch.Diff_EditCost &&
					(post_ins || post_del)) {
					// Candidate found.
					equalities[equalitiesLength++] = pointer;
					pre_ins = post_ins;
				...
				if (lastEquality && ((pre_ins && pre_del && post_ins && post_del) ||
					((lastEquality.length < DiffMatchPatch.Diff_EditCost / 2) &&
	 ...
	 * @paramdiffs Array of diff tuples.
	 * @param loc Location within text1.
	 * @return Location within text2.
	 */
	diffXIndex(diffs: Diff[], loc: number) {
		let chars1 = 0;
		let chars2 = 0;
		let last_chars1 = 0;
		let last_chars2 = 0;
	 ...
	 */
	diffFromDelta(text1: string, delta: string) {
		const diffs = [];
	...
	matchMain(text: string, pattern: string, loc: number) {
		// Check for null inputs.
	 ...
	 * @private
	 */
	matchBitap(text: string, pattern: string, loc: number) {
		if (pattern.length > DiffMatchPatch.Match_MaxBits) {
		...
		let score_threshold = DiffMatchPatch.Match_Threshold;
	...
	patchAddContext(patch: PatchObject, text: string) {
		if (text.length === 0) {
			return;
		}
		...
		while (text.indexOf(pattern) !== text.lastIndexOf(pattern) &&
			pattern.length < DiffMatchPatch.Match_MaxBits - DiffMatchPatch.Patch_Margin -
	 ...
	 */
	patchMake(a: string | Diff[], opt_b: string | Diff[], opt_c: string | Diff[]) {
		let text1: string;
		let diffs: Diff[];
		...
		for (let x = 0; x < diffs.length; x++) {
			...
			switch (diff_type) {
				...
				case DIFF_DELETE:
					...
					break;
				case DIFF_EQUAL:
					if (diff_text.length <= 2 * DiffMatchPatch.Patch_Margin &&
						patchDiffLength && diffs.length !== x + 1) {
						// Small equality inside a patch.
						patch.diffs[patchDiffLength++] = diffs[x];
						patch.length1 += diff_text.length;
						patch.length2 += diff_text.length;
					} else if (diff_text.length >= 2 * DiffMatchPatch.Patch_Margin) {
	 ...
	 * @return Array of Patch objects.
	 */
	patchDeepCopy(patches: PatchObject[]) {
	 ...
	 */
	patchApply(patches: PatchObject[], text: string): [string, boolean[]] {
		if (patches.length === 0) {
			return [text, []];
		}
		...
		for (let x = 0; x < patches.length; x++) {
			...
			if (text1.length > DiffMatchPatch.Match_MaxBits) {
				...
				// a monster delete.
				start_loc = this.matchMain(text, text1.substring(0, DiffMatchPatch.Match_MaxBits),
					expected_loc);
				if (start_loc !== -1) {
					end_loc = this.matchMain(text,
			...
			if (start_loc === -1) {
				...
				if (end_loc === -1) {
					...
					text2 = text.substring(start_loc, end_loc + DiffMatchPatch.Match_MaxBits);
				}
				if (text1 === text2) {
						...
						for (let y = 0; y < patches[x].diffs.length; y++) {
							...
							if (mod[0] === DIFF_INSERT) {  // Insertion
								...
								text = text.substring(0, start_loc + index2) +
									text.substring(start_loc + this.diffXIndex(diffs,
										index1 + mod[1].length));
							}
	 ...
	 */
	patchAddPadding(patches: PatchObject[]) {
		const paddingLength = DiffMatchPatch.Patch_Margin;
		let nullPadding = '';
	 ...
	 * @param patches Array of Patch objects.
	 */
	patchSplitMax(patches: PatchObject[]) {
		const patch_size = DiffMatchPatch.Match_MaxBits;
		for (let x = 0; x < patches.length; x++) {
			...
			while (bigpatch.diffs.length !== 0) {
				...
				while (bigpatch.diffs.length !== 0 &&
					...
					if (diff_type === DIFF_INSERT) {
						...
						diff_text = diff_text.substring(0,
							patch_size - patch.length1 - DiffMatchPatch.Patch_Margin);
						patch.length1 += diff_text.length;
						start1 += diff_text.length;
				...
				precontext =
					precontext.substring(precontext.length - DiffMatchPatch.Patch_Margin);
				// Append the end context for this patch.
				const postcontext = this.diffText1(bigpatch.diffs)
					.substring(0, DiffMatchPatch.Patch_Margin);
				if (postcontext !== '') {
	 ...
	 */
	patchToText(patches: PatchObject[]) {
		const text = [];
		for (let x = 0; x < patches.length; x++) {
			text[x] = patches[x];
		}
