...
const optimizeMoteTask = task.define('optimize-mote', task.series(
	util.rimraf('out-mote'),
	common.optimizeTask({
		src: 'out-build',
		entryPoints: moteEntryPoints,
		resources: moteResources,
		loaderConfig: common.loaderConfig(),
		out: 'out-mote',
		bundleInfo: undefined
	})
));
gulp.task(optimizeMoteTask);

const minifyMoteTask = task.define('minify-mote', task.series(
	optimizeMoteTask,
	util.rimraf('out-mote-min'),
	common.minifyTask('out-mote')
));
gulp.task(minifyMoteTask);

const core = task.define('core-ci', task.series(
	gulp.task('compile-build'),
	task.parallel(
		gulp.task('minify-mote'),
		gulp.task('minify-mote-reh'),
		gulp.task('minify-mote-reh-web'),
	)
));
gulp.task(core);
...
BUILD_TARGETS.forEach(buildTarget => {
	...
	const [mote, moteMin] = ['', 'min'].map(minified => {
		const sourceFolderName = `out-mote${dashed(minified)}`;
		const destinationFolderName = `Mote${dashed(platform)}${dashed(arch)}`;

		const moteTaskCI = task.define(`mote${dashed(platform)}${dashed(arch)}${dashed(minified)}-ci`, task.series(
			util.rimraf(path.join(buildRoot, destinationFolderName)),
			packageTask(platform, arch, sourceFolderName, destinationFolderName, opts)
		));
		gulp.task(moteTaskCI);

		const moteTask = task.define(`mote${dashed(platform)}${dashed(arch)}${dashed(minified)}`, task.series(
			compileBuildTask,
			//compileExtensionsBuildTask,
			//compileExtensionMediaBuildTask,
			minified ? minifyMoteTask : optimizeMoteTask,
			moteTaskCI
		));
		gulp.task(moteTask);
	...
	if (process.platform === platform && process.arch === arch) {
		gulp.task(task.define('mote', task.series(mote)));
		gulp.task(task.define('mote-min', task.series(moteMin)));
	}
});
