...
function packageTask(sourceFolderName, destinationFolderName) {
	...
	return () => {
		...
		const staticResources = gulp.src('resources/server/**/*', { base: 'resources/server' })
			.pipe(rename(function (path) {
				path.dirname = 'static/sources/' + path.dirname;
			}));
...
['', 'min'].forEach(minified => {
	...
	gulp.task(moteWebTaskCI);

	const moteWebTask = task.define(`mote-web${dashed(minified)}`, task.series(
		compileBuildTask,
		moteWebTaskCI
	));
	gulp.task(moteWebTask);
});
